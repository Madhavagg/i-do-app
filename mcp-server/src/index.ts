...
async function apiCall(
  ...
  try {
    ...
    if (!response.ok) {
      return { ok: false, error: (data.error as string) || `HTTP ${response.status}` };
    }
...
server.tool(
  'create_task',
  'Create a new task in your I do todo list',
  {
    title: z
      .string()
...
server.tool(
  'list_tasks',
  'List your tasks from I do with optional filters',
  {
    status: z
  ...
  async (args) => {
    ...
    if (tasks.length === 0) {
      const filterDesc = [];
      if (args.status !== 'all') filterDesc.push(`status: ${args.status}`);
      if (args.priority) filterDesc.push(`priority: ${args.priority}`);
      if (args.category) filterDesc.push(`category: ${args.category}`);
...
server.tool(
  'complete_task',
  'Mark a task as complete or incomplete in I do',
  {
